...
// Copy this to any project that needs Firebase REST API access
// =====================================================
...
export async function firebaseSync(collection: string, id: string, data: any): Promise<void> {
  await firebaseSet(`${collection}/${id}`, { ...data, syncedAt: new Date().toISOString() })
}
...
export async function firebaseBroadcast(channel: string, data: any): Promise<void> {
  await firebasePush(`broadcasts/${channel}`, { ...data, timestamp: new Date().toISOString() })
}
...
export async function firebaseSetStatus(key: string, data: any): Promise<void> {
  await firebaseSet(`status/${key}`, { ...data, updatedAt: new Date().toISOString() })
}
