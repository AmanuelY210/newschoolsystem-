...
---
Task ID: ACADEMIC-TOOLS
Agent: Main (Z.ai Code)
Task: Build API routes and portal modules for Academic Tools

Work Log:
- Read worklog.md, prisma schema, auth helpers (getCurrentUser, hasPermission), use-api hook, existing patterns from /api/students, /api/teachers, StudentsModule, nav-config.ts, PortalShell.tsx
- Updated Prisma schema to add missing relations:
  - Grade: added `exams Exam[]` relation
  - Subject: added `exams Exam[]` relation
  - Teacher: added `lessonPlans LessonPlan[]` and `timetables Timetable[]` relations
  - LessonPlan: added `teacher Teacher? @relation` (was missing)
  - Timetable: added `teacher Teacher? @relation` (was missing)
  - Exam: added `grade Grade?` and `subject Subject?` relations (were missing)
- Ran `bun run db:push` to sync schema to DB and regenerate Prisma Client

Part 1: Built 13 sets of API routes (26 files total) following the existing pattern (NextRequest, NextResponse, getCurrentUser, hasPermission, try/catch, dynamic params Promise<{id}>):

Academic tools APIs (10 required + 3 extensions):
1. /api/academic-years - GET (list with counts), POST (create, sets isCurrent = false on others if true)
2. /api/academic-years/[id] - GET (with semesters/curricula/exams), PUT, DELETE
3. /api/semesters - GET (filter by academicYearId), POST
4. /api/semesters/[id] - GET, PUT, DELETE
5. /api/curricula - GET (filter by academicYearId/subjectId, with subject+grade+academicYear), POST
6. /api/curricula/[id] - PUT, DELETE
7. /api/lesson-plans - GET (filter by subjectId/teacherId, teachers see own only, with subject+teacher), POST (auto-assigns teacherId for teacher role)
8. /api/lesson-plans/[id] - PUT, DELETE
9. /api/timetables - GET (filter by gradeId/sectionId/teacherId/academicYear, with grade+section+subject), POST
10. /api/timetables/[id] - PUT, DELETE
11. /api/exam-types - GET (with exam count), POST
12. /api/exam-types/[id] - PUT, DELETE
13. /api/exams - GET (filter by examTypeId/academicYearId/gradeId/subjectId/status, with examType+academicYear+grade+subject), POST
14. /api/exams/[id] - PUT, DELETE
15. /api/grading-systems - GET (sorted by minPercentage desc), POST
16. /api/grading-systems/[id] - PUT, DELETE
17. /api/holidays - GET (filter by type/year), POST
18. /api/holidays/[id] - PUT, DELETE
19. /api/teacher-assignments - GET (filter by teacher/grade/section/subject/year/status; teachers see own only; includes teacher+grade+section+subject+academicYear), POST
20. /api/teacher-assignments/[id] - PUT, DELETE
21. /api/grades (extended) - GET (now returns full grade data: code, educationLevel, description, status, sections, counts), POST (added create with all new fields)
22. /api/grades/[id] (new) - GET, PUT, DELETE
23. /api/sections (new) - GET (filter by gradeId, includes grade + student counts), POST
24. /api/sections/[id] (new) - PUT, DELETE
25. /api/subjects (extended) - GET (existing), POST (added create with all new fields: code, gradeId, creditHours, passingMarks, totalMarks, status)
26. /api/subjects/[id] (new) - GET, PUT, DELETE

Part 2: Built 15 'use client' portal modules in src/components/portal/modules/, each exported as a named export, using shadcn/ui (Card, Button, Input, Label, Select, Dialog, Table, Badge, Skeleton, AlertDialog), useApi/apiPost/apiPut/apiDelete from @/lib/use-api, useToast from @/components/ui/use-toast, hasPermission from @/lib/auth, teal/emerald theme, responsive design, stats cards, search/filter, create/edit dialog, delete confirmation, loading skeletons, empty states:

1. AcademicYearModule - CRUD with isCurrent toggle (auto-unsets others), status select, semester count badge
2. SemesterModule - CRUD linked to academic year, filter by year, status select
3. GradeModule - CRUD with code, level, educationLevel, description, status; sections/students counts in table
4. SectionModule - CRUD with grade link, capacity, roomNumber, classTeacher (select from teachers), status
5. SubjectModule - CRUD with code, grade link (general = no grade), creditHours, passingMarks, totalMarks, status
6. TeacherAssignmentModule - standalone module (separate from TeachersModule) for assigning teachers to grade/section/subject with academicYear, campus, weeklyPeriods, room, status; cascading dropdowns (grade→section, grade→subjects)
7. CurriculumModule - CRUD with academicYear, subject, title, description, topics (textarea, parsed per line), objectives; view dialog with parsed topic list
8. LessonPlanModule - CRUD with subject, teacher, title, description, content, date, duration, fileUrl; view dialog with full content
9. TimetableModule - CRUD with grade, section, subject, teacher, dayOfWeek, startTime, endTime, room, academicYear; rendered as weekly grid (Mon-Sat × 9 time slots) with color-coded subjects and inline edit/delete
10. ExamModule - CRUD with examType, academicYear, name, grade, subject, examDate, totalMarks, passingMarks, status (scheduled/ongoing/completed); status filter
11. GradingSystemModule - CRUD with name, minPercentage, maxPercentage, grade, gradePoint, description; color-coded grade badges (A=green, B=teal, C=blue, D=amber, F=red)
12. AttendanceSettingsModule - placeholder settings page with toggle switches, late threshold, attendance period, minimum %, parent notifications, weekend days selector, academic year dates; placeholder note
13. CalendarModule - school calendar showing holidays + events + exams merged; month grid view with event badges + upcoming events sidebar; list view sorted by date; type filter (all/holiday/event/exam); prev/next/today navigation
14. HolidaysModule - CRUD with name, date, type (holiday/event/exam), description; type filter; day-of-week column
15. ClassTeacherModule - assigns class teachers to sections via dialog (uses PUT /api/sections/[id] with classTeacherId); shows assigned teacher badges, unassigned count, assign/reassign/remove actions

Part 3: Registered all 15 new modules in PortalShell.tsx MODULE_COMPONENTS map with the IDs matching nav-config.ts:
- academic-year, semester, grades, sections, subjects, teacher-assignment, class-teacher, curriculum, lesson-plan, timetable, exams, grading-system, attendance-settings, school-calendar, holidays

Verification:
- `bun run db:push` - schema synced successfully (added 6 new relations: Grade.exams, Subject.exams, Teacher.lessonPlans, Teacher.timetables, LessonPlan.teacher, Timetable.teacher, Exam.grade, Exam.subject)
- `bun run lint` - passes with ZERO errors and ZERO warnings
- `npx tsc --noEmit` - no errors in any of the new API route files or portal module files (only pre-existing errors in dashboard/registrations/cms/seeds/examples remain unchanged)
- Dev server log shows all routes returning 200, "✓ Compiled in 320ms" with no errors

Stage Summary:
- 13 sets of CRUD API routes (26 route files) created for academic tools
- 15 portal modules created with full CRUD, search/filter, stats cards, dialogs, loading states, empty states, teal/emerald theme, responsive design
- All modules registered in PortalShell.tsx and wired to nav-config.ts Academic Tools group
- Prisma schema extended with 6 missing relations to support includes on Exam (grade, subject), LessonPlan (teacher), Timetable (teacher), Teacher (lessonPlans, timetables), Grade (exams), Subject (exams)
- Permission gating: super_admin/admin can manage all academic tools; teachers see only their own lesson plans and teacher assignments; module action buttons hidden based on hasPermission()

---
Task ID: ACADEMIC-TOOLS-MENU
Agent: Main (Claude)
Task: Implement Academic Tools menu with all academic management modules

Work Log:
- Updated Prisma schema with 8 new models: AcademicYear, Semester, Curriculum, LessonPlan, Timetable, ExamType, Exam, GradingSystem, Holiday
- Enhanced existing models: Grade (code, educationLevel, description, status), Section (roomNumber, classTeacherId, status), Subject (creditHours, passingMarks, totalMarks, status)
- Enhanced TeacherAssignment with academicYearId, campus, weeklyPeriods, room, status fields
- Updated nav-config.ts with NavItem/NavGroup types supporting collapsible groups
- Created ROLE_NAV with Academic Tools group containing 18 sub-modules
- Updated PortalLayout sidebar to render collapsible Academic Tools group with expand/collapse
- Built 26 API route files for all academic entities (academic-years, semesters, curricula, lesson-plans, timetables, exam-types, exams, grading-systems, holidays, teacher-assignments, grades, sections, subjects)
- Built 15 portal modules:
  - AcademicYearModule, SemesterModule, GradeModule, SectionModule, SubjectModule
  - TeacherAssignmentModule (standalone), ClassTeacherModule
  - CurriculumModule, LessonPlanModule, TimetableModule (weekly grid)
  - ExamModule, GradingSystemModule, AttendanceSettingsModule
  - CalendarModule (month grid + list), HolidaysModule
- Registered all new modules in PortalShell
- Verified with Agent Browser:
  - Academic Tools group expands to show all 18 sub-modules
  - Academic Year, Teacher Assignment, Timetable, Exams, Grading System, Calendar modules all load correctly
  - Existing teacher assignments visible in Teacher Assignment module
  - Lint passes cleanly

Stage Summary:
- Academic Tools menu groups all academic configuration in one place
- Super Admin can manage academic tools for all schools
- School Admin can manage academic tools for their own school
